...
def readVaultSecret(String secretPath, String secretKey) {
    echo "🔄 Reading secret from Vault: ${secretPath}"
    def secret = withVault(
        configuration: [
...
def createOpenshiftSecret(String secretPath, String secretKey) {
    echo "🔄 Creating Openshift secret from Vault: ${secretPath}"
    def secret = withVault(
        configuration: [
            vaultUrl: '',           // leave empty to use global config
            vaultCredentialId: ''   // leave empty to use global config
        ...
        vaultSecrets: [
            [path: "${secretPath}",
             secretValues: [[envVar: '${secretKey}', vaultKey: '${secretKey}']]]
            ]
        ]
    )
}
