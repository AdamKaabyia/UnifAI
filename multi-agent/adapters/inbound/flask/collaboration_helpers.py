...
def validate_team(team_id: str):
    ...
    if not authenticated:
        return jsonify({"error": "Missing authenticated user"}), 401
    if not current_app.container.identity_provider.is_member(authenticated, team_id):
...
def validate_session_access(session_id: str):
    ...
    if record.identity.type == IdentityType.TEAM:
        if not current_app.container.identity_provider.is_member(authenticated, record.identity.id):
