...
case "$ROLE" in
  flask)
    ...
    exec venv/bin/gunicorn -b 0.0.0.0:$PORT --capture-output --log-level debug --access-logformat '%(h)s %(l)s %(u)s %(t)s "%(r)s" %(s)s %(b)s pid=%(p)s "%(f)s" "%(a)s"' --access-logfile - --error-logfile - --spew bootstrap.flask_app:app
    ;;
